...
namespace popmsg
{
    ...
    public class Register
    {
        ...
        public virtual bool WriteRegeditKey(object content, RegistryValueKind rv)
        {   ///返回结果
            bool result = false;
            ...
            try
            {
                key.SetValue(_regeditkey, content, rv);
                result = true;
            }
            catch
            {
        ...
        protected virtual RegistryKey OpenSubKey()
        {
            ...
            ///打开注册表项
            sKey = key.OpenSubKey(_subkey, true);
            ///关闭对注册表项的更改
            key.Close();
            ///返回注册表节点
            return sKey;
        ...
        protected virtual RegistryKey OpenSubKey(string subKey)
        {
            ...
            RegistryKey sKey = null;
            ///打开注册表项
            sKey = key.OpenSubKey(subKey, true);
            ///关闭对注册表项的更改
            key.Close();
            ///返回注册表节点
            return sKey;
        ...
        #region 业务方法
        public static CultureInfo GetCultureInfo(string lan)
        {
            string appLanguage = lan;
            if (false)
            {
            }
            else if (appLanguage.ToLower() == "zh-CN".ToLower())
            {
                appLanguage = "zh-CN";  //中文(中华人民共和国)
            }
            else if (appLanguage.ToLower() == "zh-HK".ToLower())
            {
                appLanguage = "zh-HK";  //中文(香港特别行政区)
            }
            else
            {
                appLanguage = "";       //(Default)
            }
            return new CultureInfo(appLanguage);
        }

        public static void AddRun(string key , string exePath)
        {
            try
            {
                ClearRun(key, exePath);
                Register reg = new Register(@"SOFTWARE\Microsoft\Windows\CurrentVersion\Run", RegDomain.CurrentUser);
                string value = exePath;
                reg.WriteRegeditKey(key, value, RegValueKind.String);
            }
            catch { }
        }
        public static void ClearRun(string key , string exePath)
        {
            try
            {
                Register reg = new Register(@"SOFTWARE\Microsoft\Windows\CurrentVersion\Run", RegDomain.CurrentUser);
                string value = exePath;
                reg.DeleteRegeditKey(key);
            }
            catch { }
        }
